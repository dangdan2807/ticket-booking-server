"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProvinceService = void 0;
const vi_address_provide_entities_1 = require("./../../database/entities/vi-address-provide.entities");
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
let ProvinceService = class ProvinceService {
    constructor(provinceRepository) {
        this.provinceRepository = provinceRepository;
    }
    async findAll(pagination) {
        return await this.provinceRepository.find();
    }
    async findOneById(provinceId, option) {
        return await this.provinceRepository.findOne({
            where: Object.assign({ id: provinceId }, option),
        });
    }
    async findOneByCode(provinceCode, option) {
        return await this.provinceRepository.findOne({
            where: Object.assign({ code: provinceCode }, option),
        });
    }
    async findOneByName(provinceName, option) {
        return await this.provinceRepository.findOne({
            where: Object.assign({ name: provinceName }, option),
        });
    }
    async create(province) {
        return await this.provinceRepository.save(province);
    }
};
ProvinceService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(vi_address_provide_entities_1.Province)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], ProvinceService);
exports.ProvinceService = ProvinceService;
//# sourceMappingURL=province.service.js.map